...
STDOUT.sync = true
print "##SNMP Polling:target=\"#{target}\" mdata=\"#{mdatafile}\" community=\"#{community}\" interval=#{interval} snmp_timeout=#{snmp_timeout}\n"

# ポーリング対象MIB情報をJSONファイルから読み込む
          ...
          when 'delta','bps','pps' then ## 増分表示
            if arrayOld[i] == nil then # 直前の値が取れていない場合は'^'で表示
              ...
              if vb.value.to_i >= arrayOld[i] then
                print ",#{((vb.value.to_i - arrayOld[i]) * bits / polling_interval).round}"
              else
                print ",#{((vb.value.to_i + (wrap_value - arrayOld[i])) * bits / polling_interval).round}"
              end
            end
